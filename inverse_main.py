...
print(f"👉 Shape actuation: {actuations_raw.shape}")  # (N,9)
...
def infinity(a, b, center, N):
    t = np.linspace(0, 2 * np.pi, N)
    return np.stack([center[0] + a * np.sin(t),
                     center[1] + b * np.sin(2 * t)], axis=1), t
...
# ----- Calcolo range dinamico incl. sintetici -----
...
# qui prendo solo i primi 3 canali per il plot delle attuazioni
circle_act3 = circle_act.cpu()[:, :3]
inf_act3    = inf_act.cpu()[:, :3]

plot_colored_trajectory(circle_pts, circle_rec, t_circle,
                        "Cerchio", "plots/plot_inv/circle_workspace_colorcoded_kan.png", "KAN")
plot_colored_trajectory(infinity_pts, inf_rec, t_infinity,
                        "Infinito", "plots/plot_inv/infinity_workspace_colorcoded_kan.png", "KAN")

plot_actuations(t_circle, circle_act3,
                "Attuazioni - Cerchio (solo attuatori 1–3)", "plots/plot_inv/actuation_circle_kan_1to3.png")
plot_actuations(t_infinity, inf_act3,
                "Attuazioni - Infinito (solo attuatori 1–3)", "plots/plot_inv/actuation_infinity_kan_1to3.png")
